...
class AvCaptureTest < ApplicationSystemTestCase
  ...
  def run_capture
    ...
    MAX_ATTEMPTS.times do |attempt|
      ...
      begin
        fire_max, ice_max, frames = run_attempt(attempt)
  ...
  def run_attempt(attempt)
    ...
    return [ 0, 0, [] ] unless page.has_css?(".frontpage_title", visible: false, wait: 15)
    ...
    return [ 0, 0, [] ] unless page.has_css?(".character_selection", wait: 10)
    ...
    return [ 0, 0, [] ] unless page.has_css?(".char_name_label", minimum: 2, wait: 10)
    ...
    [ *verify_effect(baseline, frames), frames ]
  end
  ...
  def build_montage(best)
    frames = if best
               ...
               last ? Dir[File.join(last, "f-*.png")].sort : []
    end
  ...
  def emit_native_outputs(best)
    dir = if best
            ...
            Dir[CAPTURE_ROOT.join("attempt-*").to_s].sort.last
    end
    return unless dir
    ...
    # The name-label X carries a camera-scroll offset (update_char_labels
    # subtracts `background.cameraX`, a float, while the foreground layer the
    # sprite is actually drawn on is pixel-snapped with `| 0` in scroll()),
    # so the label can be tens of px off the character's true screen X.
    # Re-derive X from Firzen's distinctive gold trim in the extracted native
    # baseline; the label's Y (feet) is reliable, so it is kept.
    base_src = dir.join("base.png").to_s
    base_native = native_dir.join("base.png").to_s
    detected_x = nil
    if extract_native(base_src, base_native, viewport)
      detected_x = detect_caster_x(base_native, feet["left"].to_f, feet["top"].to_f)
    end
  ...
  # Firzen's centre-x in the native baseline, found by his distinctive gold
  # trim (hue 40-60, high saturation and value). No other object in the
  # character band shares this colour: the CPU (Davis) and the grey Stanley
  # Prison stage carry none. The search window is centred on the label X and
  # wide enough to absorb the camera-scroll error (~130px) without straying
  # into the CPU. Returns nil (caller falls back to the label X) if no gold is
  # found, so a detection miss can never break the crop.
  def detect_caster_x(native_path, label_x, label_y)
    raw = Open3.capture2("magick", native_path, "-depth", "8", "rgba:-").first.b
    x0 = [ (label_x - 130).floor, 0 ].max
    x1 = [ (label_x + 130).ceil, NATIVE_W - 1 ].min
    y0 = [ (label_y - 60).floor, 0 ].max
    ...
    (y0..y1).each do |y|
      (x0..x1).each do |x|
        ...
        if h >= 35 && h <= 65 && s > 0.45 && v > 0.5
          sum_x += x
          count += 1
        end
      end
    end
    count.zero? ? nil : (sum_x / count)
  end
  ...
  def rgb_to_hsv(r, g, b)
    ...
    if d != 0
      h = case mx
      when r then 60.0 * (((g - b) / d) % 6)
      when g then 60.0 * ((b - r) / d + 2)
      else 60.0 * ((r - g) / d + 4)
      end
      h += 360.0 if h < 0
    end
