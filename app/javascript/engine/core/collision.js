...
export default {
    ...
    \*/
    circle: function (A, B) {
      return (math.distance(A.center, B.center) <= A.radius + B.radius)
    },
    ...
    line: function (A, B, C, D) {
      const res = (math.signed_area(A, B, C) > 0 != math.signed_area(A, B, D) > 0) &&
        (math.signed_area(C, D, A) > 0 != math.signed_area(C, D, B) > 0)
      return res
    },
    ...
    point_in_rect: function (P, R) {
      return (math.inbetween(P.x, R.left, R.right) && math.inbetween(P.y, R.top, R.bottom))
    }
