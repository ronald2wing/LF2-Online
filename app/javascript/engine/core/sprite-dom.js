...
// Installs the shared set_xy / set_x_y / set_flipx / set_flipy methods for a
// given transform property (css3d vs css2d). The no-transform fallback is
// structurally different (left/top) and stays inline below.
function installTransformMethods(Sprite, property, use3d) {
  Sprite.prototype.set_xy = function (pos) {
    this.x = pos.x; this.y = pos.y
    this.el.style[property] = translateStyle(pos.x, pos.y, use3d) + (this.x_flipped ? 'scaleX(-1) ' : '') + (this.y_flipped ? 'scaleY(-1) ' : '') + scaleStyle(this.scale)
  }
  Sprite.prototype.set_x_y = function (x, y) {
    this.x = x; this.y = y
    this.el.style[property] = translateStyle(x, y, use3d) + (this.x_flipped ? 'scaleX(-1) ' : '') + (this.y_flipped ? 'scaleY(-1) ' : '') + scaleStyle(this.scale)
  }
...
if (support.css3dtransform && !Sprite._masterconfig.disable_css3dtransform) {
  installTransformMethods(Sprite, support.css3dtransform, true)
} else if (support.css2dtransform && !Sprite._masterconfig.disable_css2dtransform) {
  installTransformMethods(Sprite, support.css2dtransform, false)
} else {
  Sprite.prototype.set_xy = function (pos) {
    this.x = pos.x; this.y = pos.y
...
Sprite.prototype.set_alpha = function (alpha) {
 ...
 */
Sprite.prototype.adopt_img = function (image) {
...
// Installs the shared set_img_xy / set_img_x_y methods for a given transform
// property (css3d vs css2d). No flip flags here; the fallback stays inline.
function installImgTransformMethods(Sprite, property, use3d) {
  Sprite.prototype.set_img_xy = function (pos) {
    this.x = pos.x; this.y = pos.y
    const img = this.img[this.cur_img]
    if (img) img.style[property] = translateStyle(pos.x, pos.y, use3d)
  }
  Sprite.prototype.set_img_x_y = function (x, y) {
    this.x = x; this.y = y
    const img = this.img[this.cur_img]
    if (img) img.style[property] = translateStyle(x, y, use3d)
  }
}

if (support.css3dtransform && !Sprite._masterconfig.disable_css3dtransform) {
  installImgTransformMethods(Sprite, support.css3dtransform, true)
} else if (support.css2dtransform && !Sprite._masterconfig.disable_css2dtransform) {
  installImgTransformMethods(Sprite, support.css2dtransform, false)
} else {
  Sprite.prototype.set_img_xy = function (pos) {
    const img = this.img[this.cur_img]
    if (!img) return
