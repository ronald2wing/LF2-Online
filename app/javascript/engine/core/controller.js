...
export default class Controller {
  ...
  static keyname_to_keycode(keyname) {
    ...
    let code
    if (keyname.length === 1) {
      const firstCharCode = keyname.charCodeAt(0)
      if ((firstCharCode >= 'a'.charCodeAt(0) && firstCharCode <= 'z'.charCodeAt(0)) || (firstCharCode >= 'A'.charCodeAt(0) && firstCharCode <= 'Z'.charCodeAt(0))) {
        keyname = keyname.toUpperCase()
        code = keyname.charCodeAt(0)
      } else if (firstCharCode >= '0'.charCodeAt(0) && firstCharCode <= '9'.charCodeAt(0)) {
        code = keyname.charCodeAt(0)
      } else {
        switch (keyname) {
          case '`': code = 192; break
