...
SpriteGroup.prototype.set_alpha = function (alpha) {
  this.opacity = alpha
}
...
SpriteGroup.prototype.render = function (ctx) {
  ...
  let flipX = 0; let flipY = 0
  for (let i = 0; i < this.children.length; i++) {
    const sprite = this.children[i]
    if (!sprite.x_flipped && !sprite.y_flipped) { flip_to(0, 0) } else if (sprite.x_flipped && !sprite.y_flipped) { flip_to(1, 0) } else if (!sprite.x_flipped && sprite.y_flipped) { flip_to(0, 1) } else if (sprite.x_flipped && sprite.y_flipped) { flip_to(1, 1) }
    sprite.render(ctx)
  }
  flip_to(0, 0)
  ctx.translate(-this.x, -this.y)
  ...
  function flip_to(targetFlipX, targetFlipY) {
    if (flipX !== targetFlipX && flipY !== targetFlipY) { ctx.scale(-1, -1) } else if (flipX === targetFlipX && flipY !== targetFlipY) { ctx.scale(1, -1) } else if (flipX !== targetFlipX && flipY === targetFlipY) { ctx.scale(-1, 1) }
    flipX = targetFlipX
    flipY = targetFlipY
  }
}
