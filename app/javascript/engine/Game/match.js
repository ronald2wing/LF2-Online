...
import { createPane, showHp, transformPanel, updateBattleHud } from "engine/Game/match/hud"
...
export default class Match {
  ...
  const d = {}
  d.t = self.time.t
  d.rng = [self.randomseed.x, self.randomseed.y]
  d.fx = [self.visualeffect?.livecount ?? 0, self.brokeneffect?.livecount ?? 0]
  const live = {}
  ...
  if (Object.keys(teams).length < 2) {
    ...
    if (self.stage_mode && remainingTeam === self.player_team && !self.stage_wave_spawning && !self.stage_cleared && !self.stage_wave_pending) {
      ...
      if (self.stage_wave + 1 < waves.length) {
        ...
        setTimeout(() => { self.stage_wave_pending = false; spawnStageWave(self) }, 2500)
        return
      } else {
