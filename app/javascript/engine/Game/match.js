...
export default class Match {
  ...
  if (self.battle_mode && self.battle_config) {
    ...
    for (const team of [1, 2]) {
      ...
      self.battle_teams[team] = {
        defense: t.defense,
        units: (t.units || []).map(u => ({ id: u.id, in: u.in, reserve: u.reserve, alive: 0 }))
      }
    }
  }
  ...
  this.data.load({
    ...
    if (setting.set.weapon) {
      self.drop_weapons(setting.set.weapon)
    }
  ...
  update_char_labels() {
    // Position each character's name label below their feet. Sprite top-left is
    // (ps.sx, ps.sy + ps.sz); the sprite is ~79px tall, so feet ≈ +79px.
    const self = this
    for (const uid in self.character) {
      const c = self.character[uid]
      if (c && c.name_label && c.ps && c.health && c.health.hp > 0) {
        c.name_label.style.left = (c.ps.sx + 39) + 'px'
        c.name_label.style.top = (c.ps.sy + c.ps.sz + 79) + 'px'
      }
    }
  }
  ...
  match_event(E) {
  const self = this
  if (self.state && self.state.event) self.state.event.call(this, E)
}
    ...
    if (self.battle_mode) {
      ...
      if (char.battle_unit) {
        ...
        if (!team.units.find(u => u.id === player.unit_id)) {
          team.units.push({ id: player.unit_id, in: 0, reserve: 0, alive: 0 })
        }
      }
    }
  ...
  update_battle_hud() {
    ...
    for (const team of [1, 2]) {
      const s = stats[team]
      const name = (self.battle_config && self.battle_config.teams[team] && self.battle_config.teams[team].name) || ''
      html += '<div class="battle_hud_team battle_hud_team' + team + '">'
        + '<div class="battle_hud_stats">Man: ' + s.man + '  HP: ' + s.hp + '  Reserve: ' + s.reserve + '  Die: ' + s.die + '</div>'
        + '<div class="battle_hud_diff">' + name + '</div>'
        + '</div>'
    }
    hud.innerHTML = html
  }
