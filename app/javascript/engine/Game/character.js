...
class Character extends LivingObject {
  ...
  constructor(config, data, objectId) {
    ...
    if (self.con) {
      function combo_event(comboEvent) {
        const combo = comboEvent.name;
        switch (combo) {
          case "left":
          case "right":
            if (self.allow_switch_dir) self.switch_dir(combo);
            break;
        }
        if (
          self.combo_buffer.timeout === Gameplay.combo.timeout &&
          priority[combo] < priority[self.combo_buffer.combo]
        ) {
          // combo clash — higher priority wins
        } else {
          self.combo_buffer.combo = combo;
          self.combo_buffer.timeout = Gameplay.combo.timeout;
        }
      }
    ...
    // MP gate (F.LF behaviour): refuse to enter a frame that carries a positive
    // `mp` cost unless the character can afford it, so a special move cannot
    // start without enough MP. Frames with `mp: 0`, negative `mp` (drain-over-
    // time moves), or no `mp` are never gated. The cost is `mp % 1000`; values
    // above 1000 encode an HP cost in the high digits, which is not gated here.
    const baseTransFrame = self.trans.frame.bind(self.trans);
    self.trans.frame = (frameNumber, authority) => {
      const targetFrame = self.data.frame[frameNumber < 0 ? -frameNumber : frameNumber];
      if (targetFrame && targetFrame.mp > 0 && self.health.mp < targetFrame.mp % 1000) {
        return; // not enough MP — keep the current frame/state untouched
      }
      baseTransFrame(frameNumber, authority);
    };
  }
  ...
  combo_update() {
    const self = this;
    let combo = self.combo_buffer.combo;
    if (!combo) {
      combo = null;
    }
    ...
    let res1
    if (tar1) {
      res1 = tar1.call(self, "combo", combo);
    }
    const tar2 = self.states.generic;
    let res2
    if (!res1) {
      if (tar2) {
        res2 = tar2.call(self, "combo", combo);
      }
    }
    if (tar1) {
 ...
 */
  hit(ITR, att, attps, rect) {
    ...
    {
      accepthit = true;
      const compen = self.ps.y === 0 ? 1 : 0; // grounded targets take 1 less horizontal knockback
      ...
      ) // attacked in front
      {
        ...
        if (ef_dvx) {
          ef_dvx +=
            (ef_dvx > 0 ? -1 : 1) *
            select.lookupTableAbs(Gameplay.defend.absorb, ef_dvx);
        }
        ef_dvy = 0;
  ...
  // The itr's attack volume, flattened onto the ground plane (zwidth 0), and
  // every body it overlaps. Callers that score hits keep the volume.
  body_hits(ITR) {
    const vol = this.mech.volume(ITR);
    vol.zwidth = 0;
    return { vol, hit: this.scene.query(vol, this, { tag: "body" }) };
  }
  ...
  post_interaction() {
    ...
    // Original LF2 selects a single winning itr per target when several itrs
    // overlap it: the closest itr wins and equal distances are broken with a
    // seeded 50/50 (openlf2 func_417400_does_attack_success). In practice all
    // itrs touching one target share the same distance, so the first itr wins
    // and each later overlapping itr has a 50% chance to replace it. Select
    // every winner first, then apply them once below.
    const winner = {}; // target uid -> { ITR, vol, target }

    for (const i in ITR_LIST) {
      const ITR = ITR_LIST[i]; // the itr tag in data
      const { vol, hit } = self.body_hits(ITR);
        ...
        case IK.FALLING: // falling
          for (const t in hit) {
            ...
            switch (ITR.effect) {
              case EFF.NORMAL:
              case EFF.BLOOD:
                if (target.type === "character" && target.team === self.team) {
                  // cannot attack characters of same team, except those frozen
                  // or being caught (same-team hits still land on them)
                  if (target.state() !== S.FROZEN && target.state() !== S.BEING_CAUGHT) {
                    canhit = false;
                  }
                }
                break;
              ...
              case EFF.WEAK_FIRE4:
                ...
                if (self.state() === S.BURNING && target.team === self.team) {
            ...
            if (ITR.kind === IK.FALLING) {
              if (
                self.itr.attacker.uid === target.uid || // does not hit who blown you away
                (self.itr.attacker.parent &&
                  self.itr.attacker.parent.uid === target.uid) || // specialattack
                (self.itr.attacker.hold &&
                  self.itr.attacker.hold.pre &&
                  self.itr.attacker.hold.pre.uid === target.uid)
              ) {
                // weapon
                canhit = false;
              }
            ...
            if (canhit) {
              // single winner per target: first itr wins, each later
              // overlapping itr has a 50% seeded-random chance to replace it
              if (!(target.uid in winner) || self.match.random() < 0.5) {
                winner[target.uid] = { ITR, vol, target };
              }
            }
          }
          break;
    ...
    // apply each selected winner exactly once
    for (const uid in winner) {
      const { ITR, vol, target } = winner[uid];
      if (!self.itr.arest) {
        try {
          const hitResult = target.hit(
            ITR,
            self,
 ...
 */
  ...
  caught_throw(cpoint, vdir) {
    ...
    if (cpoint && cpoint.vaction !== undefined) {
  ...
  caught_release() {
    ...
    // release toss: small fixed velocity (x +3, y -3 = up), applied once by
    // the effect tick
    self.effect.dvx = 3;
    self.effect.dvy = -3;
