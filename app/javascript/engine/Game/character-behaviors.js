...
import { transformPanel } from "engine/Game/match/hud"

const ID = Global.gameplay.ID

// Builds the replacement-character spec every "become another character" path
// needs — Rudolf's transform and its revert (which carry `transform_character`
// so the transform state survives) and Louis' armour pickup into LouisEX (which
// does not). `name` is the label shown while the replacement loads; only the
// name, target id and transform carry-over differ between callers.
export function createTransform(self, id, name, transformCharacter) {
  const spec = {
    dir: self.ps.dir,
    health: self.health,
    stat: self.stat
  }
  if (transformCharacter !== undefined) spec.transform_character = transformCharacter
  spec.replace_from = self
...
const characterBehaviors = {
  default(event, K, tag) {
    if (event === "revert_transform") {
      ...
      transformPanel(self.match, self.uid)
      createTransform(self, ID.TRANSFORM, "transform", self.transform_character)
    }
  },
  ...
  [ID.RUDOLF](event) {
    ...
    switch (event) {
      ...
      case "rudolf_transform":
        ...
        transformPanel(self.match, self.uid, self.transform_character.uid)
        createTransform(self, self.transform_character.id, "transform", self.transform_character)
        break
    ...
    switch (event) {
      ...
      case "pickup_when_holding":
        ...
        if ((heldId === ID.ARMOR_PIECE_1 || heldId === ID.ARMOR_PIECE_2) &&
          ...
          self.hold.obj = null
          createTransform(self, ID.LOUIS_EX, "LouisEX")
          return true
            }
        return false
    }
