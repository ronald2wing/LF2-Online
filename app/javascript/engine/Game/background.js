...
function decodeColor(rect) {
  if (typeof rect === 'string') {
    ...
    const lookup = global.background.colors[rect]
...
// Native LF2 viewport width, captured once at module import. The app is fixed
// at 794×550 (see global.js), so these never change at runtime and the TU()
// camera math below can safely use the cached values.
...
export default class Background {
  constructor(config, data, id) {
    ...
    {
      // Load the shadow image only to measure its dimensions for the shadow
      // offset. Resolve and load the image directly (no discarded sprite) and
      // remove the listeners on both load and error so a failed load cannot
      // leak the image or its listeners.
      const img = new Image()
      function onload() {
      ...
      function cleanup() {
        ...
        img.removeEventListener('error', cleanup, true)
      }
      img.addEventListener('load', onload, true)
      img.addEventListener('error', cleanup, true)
      img.src = spriteRenderer.resolve_resource(data.shadow)
    }
    ...
    // HK Coliseum's crowd and floor layers sit 8px lower than the `back1`
    // backdrop in the source data, leaving a seam; shift every layer except
    // `back1` up 8px to close it.
  ...
  destroy() {
    ...
    for (const layer of self.layers) layer.sp.remove()
    for (const layer of self.timedLayers) layer.sp.remove()
    if (self.scrollbar) {
  ...
  leaving(obj, margin) {
    ...
    const nextX = obj.ps.sx + obj.ps.vx
    const nextY = obj.ps.sy + obj.ps.vy
    return (nextX + obj.sp.width < 0 - margin || nextX > self.width + margin || nextY < -600 || nextY > 100)
  }
  ...
  TU() {
    ...
    if (!self.cameraLocked) {
      ...
      let facing = 0
      let numPlayers = 0
      for (const uid in self.char) {
        avgX += self.char[uid].ps.x
        facing += self.char[uid].dirh()
        numPlayers++
      }
      if (numPlayers > 0) {
        avgX /= numPlayers
