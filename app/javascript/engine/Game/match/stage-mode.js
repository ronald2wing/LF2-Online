...
// The playable heroes a "random hero" wave slot (id 1000 in the official
// stage.dat) can resolve to.
const RANDOM_HEROES = [1, 2, 4, 5, 6, 7, 8, 9, 10, 11]
...
export function spawnStageWave(match) {
  ...
  for (let i = 0; i < wave.enemies.length; i++) {
    ...
    const eid = enemy.random ? RANDOM_HEROES[Math.floor(self.random() * RANDOM_HEROES.length)] : enemy.id
