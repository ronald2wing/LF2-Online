...
class GameManager {
    ...
    character_selection:
    {
            ...
            enter: function () {
              ...
              if (manager._battle_mode_pending) {
                ...
                manager._battle_leaders = players.filter(p => p.use).map(p => ({
                  ...
                  selected: p.selected === -1 ? Math.floor(randomseed.next() * char_list.length) : p.selected,
      ...
      battle_setup:
      {
        ...
        _build_team: function (team, teamConfig, selected) {
          ...
          dv.textContent = '×' + battle_defenses[teamConfig.defense]
          ...
          followerLabel.innerHTML = '<span>Follower (士兵)</span><span class="battle_diff">' + battle_presets[teamConfig.preset].name + '</span>'
          col.appendChild(followerLabel)
          const presetUnits = battle_presets[teamConfig.preset].units
          const unitList = battle_unit_types.map(type => presetUnits.find(p => p.id === type.id) || { id: type.id, in: 0, reserve: 0 })
  ...
  this.start_battle_match = function () {
    ...
    for (const team of [1, 2]) {
      ...
      teamCfg.units = preset.units.filter(u => u.id < 100)  // exclude Milk (122)/Beer (123) — items, not respawnable soldiers
  ...
  this.replay_recording = function (recording) {
    ...
    match.onframe = function (m) {
      const timeEl = util.queryUI('playback_time')
      if (timeEl) {
