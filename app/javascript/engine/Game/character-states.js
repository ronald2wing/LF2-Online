...
function updateDisappearCounter(self) {
  const count = self.counter.disappear_count
  if (count < 0) return
  if (count < Gameplay.effect.disappear.shadow_blink) {
    self.counter.disappear_count += 1
  } else if (count < Gameplay.effect.disappear.body_blink) {
    self.counter.disappear_count += 1
    ...
    // Checked after the body_blink range case so the exact threshold lands here,
    // not in the blink-range branch.
    self.counter.disappear_count += 1
...
function updateDeadBlinkCounter(self) {
  const count = self.counter.dead_blink_count
  if (count < 0) return
  if (count == 0) {
    self.effect.blink = true
...
export const states = {
        ...
        if (ps.y === 0 && ps.vy === 0 && self.frame.N === 212 && self.frame.PN !== 211) {
          ...
          if (result) {
            ...
            self.mech.linear_friction(
              select.lookupTableAbs(Gameplay.friction.fell, ps.vx),
              select.lookupTableAbs(Gameplay.friction.fell, ps.vz)
            )
          }
      ...
      case "combo":
        switch (combo) {
          ...
          default:
            ...
            const tag = Global.combo_tag[combo]
            if (tag && self.frame.D[tag]) {
              if (!self.behavior("generic_combo", combo, tag)) {
                const dir = Global.combo_dir[combo]
          ...
          case null: {
            ...
            if (dx || dz) {
              if (self.hold.obj && self.hold.obj.type === "heavyweapon") {
                ...
                if (combo !== "jump") self.trans.frame(5, 5)
  ...
  [S.WALKING](event, combo) {
    ...
    switch (event) {
      ...
      case "combo":
        ...
        if (combo) return self.states[S.STANDING].call(self, event, combo)
        break
    }
  },

  [S.RUNNING](event, combo) {
  ...
  [S.JUMP](event, combo) {
    ...
    switch (event) {
      ...
      case "TU":
        ...
        break
      case "combo":
        if ((combo === "att" || self.con.state.att) && !self.statemem.attlock) {
          if (self.frame.N === 212) {
            if (self.hold.obj) {
              const dx = self.con.state.left !== self.con.state.right
  ...
  [S.DASH](event, combo) {
    ...
    switch (event) {
      case "state_entry":
        ...
        break
      case "combo":
        if (combo === "att" || self.con.state.att) {
          if (
    ...
    switch (event) {
        ...
        if (self.effect.dvy <= 0) {
          switch (self.frame.N) {
            case 180:
              ...
              self.trans.setWait(select.lookupTableAbs(Gameplay.fall.wait180, self.effect.dvy))
              break
            case 181: {
      ...
      case "fall_onto_ground":
        ...
        if (
          ...
          self.mech.linear_friction(
            select.lookupTableAbs(Gameplay.character.bounceup.absorb, ps.vx),
            select.lookupTableAbs(Gameplay.character.bounceup.absorb, ps.vz)
          )
  ...
  [S.DANCE_OF_PAIN](event, combo) {
    // injured 2 (dance of pain)
  },

  [S.DRINKING](event, combo) {
    ...
    switch (event) {
      case "state_entry":
        ...
        if (self.hold.obj && self.hold.obj.type === "drink") {
          self.hold.obj.tickN = 0
          self.hold.obj.hpDone = 0
          self.hold.obj.mpDone = 0
        }
        break
      case "TU":
        // Recovery is metered per game tick, not per animation frame. The
        // drinking frames are wait: 3 (4 TUs each), so ticking on "frame"
        // would run the whole drink four times too slowly.
        if (self.hold.obj && self.hold.obj.type === "drink") {
          const drink = self.hold.obj
          // The drink is destroyed and the state ends once its totals are met.
          if (drink.tick(self)) {
            self.hold.obj = null
  ...
  [S.BURNING](event, combo) {
    ...
    switch (event) {
      ...
      case "fell_onto_ground":
        return self.states[S.FALLING].call(self, event, combo)
    }
  },

  [S.FIRERUN](event, combo) {
    // firen specific
    const self = this
    switch (event) {
      case "TU":
