...
class FrameTransition {
  ...
  // Every write to #wait / #next goes through the same authority gate: a
  // caller at a lower authority than the current lock is ignored, and the
  // lockout counter is reset (inheriting the old wait when asked to).
  #acquire(authority, out) {
    if (authority === AUTH.INHERIT) authority = this.#lock
    if (authority < this.#lock) return false
    this.#lock = authority
    this.#lockout = out === AUTH.INHERIT ? this.#wait : out
    return true
  }

  setWait(value, authority = 0, out = 1) {
    if (!this.#acquire(authority, out)) return
    this.#wait = Math.max(0, value)
  }

  incrementWait(inc, authority = 0, out = 1) {
    if (!this.#acquire(authority, out)) return
    this.#wait = Math.max(0, this.#wait + inc)
  }
...
export default class LivingObject {
  ...
  frame_force() {
    ...
    // LF2 frame data uses dvx/dvy/dvz = 550 as a "stop" marker (e.g. lying
    // frames freeze the character in place). It is far above any real
    // velocity, but the force application above would still treat it as one,
    // so these checks zero the axis afterwards.
  ...
  vol_itr(kind) {
    ...
    return self.mech.body_itr(kind)
  }
