...
export class VerticalMenuDialog {
  constructor(config) {
    ...
    if (config.mousehover) {
      const self = this
      this.dia.el.onmousemove = (e) => {
        const pt = _mouseTrans(this.dia.el, e)
        self._mousemove(pt.x, pt.y)
      }
...
export class SummaryDialog {
  ...
  setRowData(i, data) {
    ...
    for (let j = 1; j < data.length; j++) row[j - 1].textContent = data[j]
...
// Shared keyboard-navigation lifecycle for the menu/setup screens. Each screen
// yields keyboard input to the gameplay controller while active (so game keys
// don't leak into the menus) and binds a document keydown handler via
// ctx.addKeyNav for the duration of its activation.

// Wire a vertical-menu dialog (stage_select, demo_setup) to the keyboard:
// ↑↓/wx navigate, ←→/ad cycle the active item's value, Enter/s activates,
// Escape returns to the frontpage. `screen` must own a `dialog` and an
// `_activate(item)` method; this attaches its `onactive`/`deactive`.
export function dialogKeyNav(ctx, screen) {
  screen.onactive = function () {
    inputController.block(false)
    ctx.addKeyNav(screen, (e) => {
      switch (e.key) {
        ...
        case 'ArrowDown': case 'x': case 'X':
          e.preventDefault()
          screen.dialog.navDown()
          break
        case 'ArrowLeft': case 'a': case 'A':
          e.preventDefault()
          if (screen.dialog.valueCounts[screen.dialog.active_item]) {
            screen.dialog.cycleValue(screen.dialog.active_item, -1)
          }
          break
        ...
        case 'Enter': case 's': case 'S':
          e.preventDefault()
          screen._activate(screen.dialog.active_item)
          break
        case 'Escape':
  ...
  screen.deactive = function () {
    inputController.block(true)
    ctx.removeKeyNav(screen)
  }
}

// The onactive/deactive skeleton shared by screens that rebuild their DOM on
// activation and handle their own keys (championship, battle_setup): unblock
// input, rebuild, and bind `handler` (invoked with `this` = `screen`).
export function bindKeyNav(ctx, screen, handler) {
  screen.onactive = function () {
    inputController.block(false)
    screen._build()
    ctx.addKeyNav(screen, (e) => handler.call(screen, e))
  }
  screen.deactive = function () {
    inputController.block(true)
    ctx.removeKeyNav(screen)
