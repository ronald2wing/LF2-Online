...
export function createNetworkGame(ctx) {
  return {
    bgcolor: ctx.pack.data.UI.data.network_game.bg_color,
    create: function () {
      const This = this
    ...
    connect_to: function(addr) {
      ...
      if (!addr) return
      addr = addr.replace(/\/$/, '')
      // A bare host (no scheme) would resolve relative to this page; prefix
      // https so the probe hits the intended server instead of this origin.
      if (!/^[a-zA-Z][a-zA-Z0-9+.-]*:\/\//.test(addr)) {
        addr = 'https://' + addr
      }
