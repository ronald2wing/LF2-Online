...
export function createBattleSetup(ctx) {
  const screen = {
    ...
    _build_team: function (team, teamConfig, selected) {
      ...
      const leaders = (ctx.manager._battle_leaders || []).filter(L => L.team === team)
      for (let i = 0; i < 4; i++) {
        const L = leaders[i]
        const slot = document.createElement('div')
        slot.className = 'battle_hero_slot'
        if (L) {
          const sel = (L.selected >= 0 && ctx.char_list[L.selected]) ? ctx.char_list[L.selected] : null
          if (sel && sel.pic) {
    ...
    _key: function (e) {
      ...
      switch (e.key) {
        ...
        case 'w': case 'W': case 'ArrowUp':
          e.preventDefault()
          cfg.teams[cfg.selectedTeam].defense = (cfg.teams[cfg.selectedTeam].defense + 1) % ctx.battle_defenses.length
          this._build()
          break
        case 'x': case 'X': case 'ArrowDown':
          e.preventDefault()
          cfg.teams[cfg.selectedTeam].defense = (cfg.teams[cfg.selectedTeam].defense + ctx.battle_defenses.length - 1) % ctx.battle_defenses.length
          this._build()
          break
        case 'q': case 'Q':
          e.preventDefault()
          cfg.teams[cfg.selectedTeam].preset = (cfg.teams[cfg.selectedTeam].preset + 1) % ctx.battle_presets.length
          this._build()
          break
