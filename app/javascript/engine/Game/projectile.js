...
export function firzenIceColumns(self) {
  ...
  // Each column plays its data growth chain (~12 frames) and would otherwise
  // rest on frame 5/55's wait:175 (~5.8s) before shattering — the original's
  // ice instead melts ~1.9s after the cast. The columns spawn ~25 frames after
  // the cast (the disaster ball's hit_Fa 11 fires on its 2nd update), so a
  // 32-frame lifetime from spawn breaks them at ~1.9s: grow, then the frame-40
  // burst + brokeneffect shard cloud (see the generic TU melt countdown).
  const melt = 32
  const columns = [
  ...
  for (const { x, facing, action, ice } of columns) {
    const spacing = ice * s
    cells.push({ kind: 1, x, y: 27, action, dz, dvx: 0, dvy: 0, oid: 212, facing, melt })
    cells.push({ kind: 1, x, y: 27 - spacing, action, dz, dvx: 0, dvy: 0, oid: 212, facing, melt })
  }
  return cells
}
...
// The base flame tongues under the eruption. The reference GIF's fire forms a
// broad band at the caster's feet — spanning ≈±107px and reaching ≈20px below
// the feet line — but the old five-flame set (x ±40px, y 27) was a sparse strip
// that sat ~6px above the feet and never touched the ground. Seven flames at
// 30px spacing span ±90px; y 55 drops the band so the flames sit on the ground
// and dip ~20px below the feet line. Alternating ground-fire actions (50 =
// looping spreading flame, 70 = short dying flame) keeps a ragged top.
export function firzenBaseFlames() {
  const flames = []
  for (let i = -3; i <= 3; i++) {
    flames.push({
      kind: 1, x: 42 + i * 30, y: 55,
      action: i % 2 === 0 ? 50 : 70,
          ...
          case 9: // firzen_chasef + firzen_chasei (Arctic Volcano barrage)
            ...
            if (!self.hitFa9_spawned) {
              ...
              for (let i = 0; i < ballCount; i++) {
                ...
                // and flies outboard with a spread of speeds. It launches
                // steeply upward (dvy −18) so its arc apex clears the fire
                // explosion's ~155px top (≈5.5W) — the decoded original's
                // airborne ice field reaches 2.75–5.4W, above the explosion,
                // and a −4 launch rose to only ~83px (≈3W), inside the
                // explosion, so the balls blended into it and never read as
                // separate projectiles. The steeper arc also keeps them
                // airborne longer before gravity pulls them back to the ground
                // (see the generic TU above), where they land at a distinct
                // outboard x and convert on ground contact (fire -> ground
                // flame, ice -> column). noChase keeps the flight ballistic so
                // landing positions are deterministic under lockstep (the
                // flying frames carry hit_Fa: 7, which would otherwise home
                // them onto a single target).
                ...
                self.match.create_object({
                  ...
                  oid: fire ? 221 : 222, facing: facing, noChase: true
                }, self)
              }
              self.hitFa_spawned++
            }
  ...
  generic: function (event, K) {
    ...
    switch (event) {
      case 'TU':
        ...
        // straight lines), but the hit_Fa 9 barrage balls (noChase) launch
        // upward and must arc back down to land: apply gravity so they rise
        // from the caster, peak, and fall to the ground where they convert
        // into columns/flames. Deterministic (constant gravity), so lockstep
        // peers derive identical trajectories.
        ...
        // must not consume it — the hit_Fa 9 barrage balls (firzen_chasei,
        // noChase) rely on this to fly a ballistic arc instead of self-
        // destructing mid-flight.
        if (self.frame.D.hit_a && !self.noChase) {
          self.health.hp -= self.frame.D.hit_a
        }
        // Arctic Volcano ice columns melt on a fixed lifetime instead of
        // resting on frame 5/55's wait:175 (~5.8s). When the countdown
        // expires, break to the frame-40 "disappear" burst (pic 6 shard
        // cloud); leaving state 3000 spawns the brokeneffect ice shards, so
        // the column shatters and decays instead of holding static.
        if (self.meltFrames > 0 && --self.meltFrames === 0) {
          self.trans.frame(40)
        }
        break
      ...
      case 'frame':
        if (self.frame.D.opoint) {
          ...
          // Arctic Volcano barrage: a landed ice ball (firzen_chasei, spawned
          // by hit_Fa 9 with noChase) opoints freeze_column@70 — the upright
          // icicle. The original's columns lean outward, so spawn the leaning
          // variant (action 50) with its tip pointing away from the caster
          // instead. Mirror only the columns that land out on a flank, which
          // keeps the random fire/ice split balanced on both sides. self.parent
          // is the stationary disaster at the eruption centre (the caster's
          // position, the same axis the fixed columns mirror about). noChase
          // excludes Firzen's disaster-move ice ball, which also lands as a
          // column but is a single aimed shot.
          if (self.noChase && op.oid === 212 && op.action === 70 && self.parent?.ps) {
            const axis = self.parent.ps.x
            const colX = self.mech.make_point(op).x
            // action 50's ice tip sits ~34px to the right of its base within
            // the cell. facing 0 keeps the ice ball's direction (left, mirrored
            // lean → tip left), facing 1 reverses it (tip right), so this points
            // the tip away from the caster on whichever side the column lands.
            const facing = colX < axis ? 0 : 1
            self.match.create_object({ ...op, action: 50, facing }, self)
            // Mirror only the outboard landings. The slowest fan ball lands
            // ≈150px from the caster, so a 90px radius still mirrors every
            // barrage column while refusing to mirror a near-caster landing
            // (a safety net against reintroducing a middle column).
