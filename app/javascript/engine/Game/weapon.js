...
// A thrown weapon (ITR kind 0) and a held-weapon swing (kind 5) both follow the
// normal-hit team rules: a same-team target is skipped unless it is a
// weapon/drink, frozen, or being caught.
function can_hit_teammate(self, target) {
  if (target.team !== self.team) return true
  return target.type === "lightweapon" ||
    target.type === "heavyweapon" ||
    target.type === "drink" ||
    target.state() === S.FROZEN ||
    target.state() === S.BEING_CAUGHT
}
      ...
      switch (event) {
        case 'frame':
          if (self.frame.N === 64) { // on ground
            ...
            self.trans.setNext(self.frame.N, 99) // rest: hold this frame
          }
          break
      }
    },
  ...
  class TypeWeapon extends LivingObject {
    ...
    interaction() {
      ...
      for (const j in itrs) {
        ...
        const targets = self.scene.query(vol, self, { tag: 'body' })
        ...
        for (const k in targets) {
          ...
          if (!can_hit_teammate(self, target)) continue
    ...
    act(att, wield, point) {
      ...
      if (self.light && wield.attacking) {
        ...
        for (const j in itrs) {
          ...
          const vol = self.mech.volume(itr)
          vol.zwidth = 0
          const targets = self.scene.query(vol, [self, att], { tag: 'body' })
          ...
          for (const k in targets) {
            ...
            if (!self.attacked(target.hit(citr, att, { x: att.ps.x, y: att.ps.y, z: att.ps.z }, vol))) continue
