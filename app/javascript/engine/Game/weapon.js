...
function Weapon(type) {
  ...
  class TypeWeapon extends LivingObject {
    ...
    interaction() {
      ...
      for (const j in itrs) {
        ...
        for (const k in targets) {
          ...
          const rest = (itr.arest !== undefined || itr.vrest !== undefined) ? itr : Gameplay.default.weapon
    ...
    itr_arest_update(ITR, uid, obj) {
      const self = this
      // Base-class compat: called with just ITR (standard signature)
      if (uid === undefined) {
        this.itr.arest = ITR?.arest ?? Gameplay.default.character.arest
        return
      }
      // Extended signature: (ITR, uid, obj) for weapon-specific vrest logic
      let rest
      if (ITR?.arest) {
        rest = ITR.arest
      } else if (ITR?.vrest) {
        ...
        rest = Gameplay.default.weapon.vrest
      }
      if (obj?.type === 'heavyweapon' || obj?.type === 'lightweapon') {
        rest *= 2
      }
      self.itr.vrest[uid] = rest
    }
