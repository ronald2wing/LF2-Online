    ...
    interaction() {
      ...
      for (const j in itrs) {
        ...
        for (const k in targets) {
          ...
          // A kind-0 thrown weapon follows the normal-hit team rules: same-team
          // targets are skipped unless frozen, caught, or a weapon/drink.
          if (target.team === self.team &&
              target.type !== "lightweapon" &&
              target.type !== "heavyweapon" &&
              target.type !== "drink" &&
              target.state() !== S.FROZEN &&
              target.state() !== S.BEING_CAUGHT) continue
      ...
      if (self.light && wield.attacking) {
        ...
        for (const j in itrs) {
          ...
          for (const k in targets) {
            ...
            const target = targets[k]
            // A kind-5 swing follows the normal-hit team rules: same-team
            // targets are skipped unless frozen, caught, or a weapon/drink.
            if (target.team === self.team &&
                target.type !== "lightweapon" &&
                target.type !== "heavyweapon" &&
