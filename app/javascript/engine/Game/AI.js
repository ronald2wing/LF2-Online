...
// Per-frame field kinds the AI reads: `bdy`/`itr` are wrapped into arrays
// (with a `<field>_count`), `wpoint` is passed through as an object.
const FRAME_FIELD_KINDS = { bdy: "make_array", itr: "make_array", wpoint: "object" }
...
class AIInterface {
  ...
  frame(N) {
    ...
    if (this.self.data.frame[N]) {
      ...
      for (const t in FRAME_FIELD_KINDS) {
        if (FRAME_FIELD_KINDS[t] === "make_array") O[t + "_count"] = 0
      }
    }
    return O
  }
