...
export default class Scene {
  ...
  // Swap an entity in place, keeping its uid so panels and cross-references to
  // the old object stay valid. The key already exists (uids are integers, so
  // for..in enumerates them numerically), hence a plain assignment.
  ...
  query(volume, exclude, where = {}) {
    ...
    // Iterate the live map directly by key: Object.values(this.live) allocates
    // a fresh array on every hit test (an O(n) allocation inside an O(n)
    // broadphase). Keys are integer uids, so for..in visits them in the same
    // ascending order Object.values would.
    for (const key in this.live) {
      const entity = this.live[key]
