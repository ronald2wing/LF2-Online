...
const channels = {
  ...
  messenger: {
    send: makeSender("m"),
    receive(msg) {
      if (link.messenger && link.messenger.receiver) {
        link.messenger.receiver.onmessage(msg)
      } else {
        console.warn("dropping message: " + msg)
      }
    },
  },
  transfer: {
    ...
    on(event, data) {
      switch (event) {
        case "open":
          peer = data
          link.open = true
          link.messenger.send = channels.messenger.send
          channels.frame.send({})
          handler.on("open")
          break
        case "close":
