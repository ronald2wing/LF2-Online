...
function startClock(callback, interval) {
  ...
  clockId = setInterval(onTick, interval)
  return clockId
}
...
class InputProxy {
  ...
  wrap(control) {
    ...
    const skip = { clearStates: 1, flush: 1, fetch: 1, key: 1 }

    for (const key in control) {
      if (typeof control[key] === "function" && !skip[key]) {
        this[key] = function () { control[key].apply(control, arguments) }
      }
    }
  }
...
// Shared by the transport loader below and the network-game UI (network_game.js).
// Strips one trailing slash and prefixes https:// to a schemeless host so it
// becomes an absolute URL instead of a path relative to this origin.
// Already-schemed values (http/https/ws/wss) are left untouched.
