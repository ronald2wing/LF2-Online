...
Each player's input per frame is a list of key events — `[key, down]` pairs
(`network.js:398`), where `key` is a controller config key name. The key names
are not hardcoded: the controller builds its keycode table from
`Object.keys(this.config)` (`controller.js:34`), so any configured key is
transmittable. The default config uses `up`, `down`, `left`, `right`, `def`,
`jump`, `att` (`manager.js:344`):

```
[ "att", 1 ], [ "left", 0 ], ...   // key name + press (1) / release (0)
```

Events are buffered on key press/release and flushed each frame. Held state
(`con.state`) persists while the key is held and is rebuilt peer-side by applying
the events.
