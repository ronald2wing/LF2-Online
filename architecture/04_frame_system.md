...
> Source: original LF2 v2.0a `.dat` frame data, `living-object.js` frame transition engine
...
| `vrest` | number | Victim rest / re-hit window (TUs) |
...
From `living-object.js` — the frame transition state machine:
...
| 6 | COUNTER | Counter hitbox on broken-defend and dance-of-pain frames; a punch landing on it forces the attacker into the super punch (frame 70). |
...
```
 ...
 220 – 225: Injured             (state 11)
 226 – 229: Dance of Pain       (state 16)
