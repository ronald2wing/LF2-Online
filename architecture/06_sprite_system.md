...
A sheet's **pitch is `(w + 1) × (h + 1)`**, not `w × h`: LF2 puts a **1px separator line** between cells, and only the `w × h` area belongs to the frame.

The separator is not transparency — it is a solid colour that varies per sheet (red `(255,1,1)` in the weapon sheets, green `(184,248,104)` in the bat sheets, blue `(99,130,248)` in the ball sheets, black in most character sheets). It is visible in the original BMPs.

Two rules follow for any renderer:

- advance one cell by `w + 1` horizontally and `h + 1` vertically;
- draw exactly `w × h`. Adding a `+1` to the drawn size renders the separator as a coloured border around every sprite — the original engine slices exactly `w × h`.
...
- `head` / `*_f.bmp`: Face avatar sprite (120 × 120 px), displayed in character select screen
- `small` / `*_s.bmp`: Small HP bar icon (40 × 45 px), displayed above character in battle

## Render Position

From `mechanics.js`, the sprite anchor is derived from the frame's `centerx`/`centery`, then depth-scaled by `z`:

```
sx = (facing right) ? x - centerx : x + centerx - sprite.w
sy = y - centery
scale = depth_scale(z)               // positive z (foreground) ⇒ scale > 1
screen_x = sx * scale + x * (1 - scale)
screen_y = (sy + z) * scale + (y + z) * (1 - scale)
```

`(x, y)` maps to the character's feet; `z` scales the sprite and shifts it down-screen (positive `z` = foreground).
