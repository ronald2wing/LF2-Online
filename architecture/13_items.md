...
| 6 | Drink | Drunk in state 17 — recovers HP/MP over time, then destroyed |
...
Held in state 17, a drink recovers its owner **gradually** — one tick per game
frame — until its totals are delivered; it is not an instant heal. The drink is
destroyed once its totals are met.

### Milk (id: 122) — `weapon6.dat`
- `weapon_hp`: 450
- `weapon_drop_hurt`: 35
- Restores **160 HP** and raises the potential (dark) bar by **80** — 8 HP and 4
  potential every 5 ticks — plus **166 MP** (5 every 3 ticks). Runs ~100 ticks.
- Sprite: 48×48

### Beer (id: 123) — `weapon8.dat`
- `weapon_hp`: 450
- `weapon_drop_hurt`: 35
- Restores **750 MP** (6 every tick), which is more than a full bar. Runs ~125 ticks.
- Sprite: 48×48
